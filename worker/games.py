...
REPO_URL = 'https://github.com/official-chameleon/books'
...
def adjust_tc(tc, base_nps, concurrency):
  ...
  # if base_nps < 700000:
    # sys.stderr.write('This machine is too slow to run chtest effectively - sorry!\n')
    # sys.exit(1)
...
def parse_cutechess_output(p, remote, result, spsa, spsa_tuning, games_to_play, batch_size, tc_limit):
  ...
  while datetime.datetime.now() < end_time:
    ...
    # Parse line like this:
    # Finished game 1 (chameleon vs base): 0-1 {White disconnects}
    ...
    # Parse line like this:
    # Score of chameleon vs base: 0 - 0 - 1  [0.500] 1
    if 'Score' in line:
      chunks = line.split(':')
...
def run_games(worker_info, password, remote, run, task_id):
  ...
  engines = glob.glob(os.path.join(testing_dir, 'chameleon_*' + EXE_SUFFIX))
  if len(engines) > 25:
  ...
  cutechess = os.path.join(testing_dir, 'sylvan-cli' + EXE_SUFFIX)
  ...
  if not os.path.exists(cutechess):
    if len(EXE_SUFFIX) > 0: zipball = 'sylvan-cli-win.zip'
    else: zipball = 'sylvan-cli-linux-%s.zip' % (platform.architecture()[0])
    setup(zipball, testing_dir)
    zip_file = ZipFile(zipball)
    zip_file.extractall()
    zip_file.close()
  ...
  # If Nodes Time is being used, give engines extra grace time to
  ...
  while games_remaining > 0:
    ...
    cmd = [ cutechess, '-repeat', '-rounds', str(int(games_to_play/2)), '-games', ' 2', '-tournament', 'gauntlet'] + pgnout + \
          ['-site', 'http://chtest.org/tests/view/' + run['_id']] + \
