      ...
      if not 'spsa' in run['args']:
        try:
          with open(pgn_file, 'r') as f:
            data = f.read()
          payload['pgn'] = base64.b64encode(zlib.compress(data.encode('gb2312'))).decode()
...
def main():
  ...
  config_file = path.join(worker_dir, 'chtest.cfg')
  ...
  if cpu_count <= 0:
    sys.stderr.write('Not enough CPUs to run chtest (it requires at least two)\n')
    sys.exit(1)
